...
struct Context
{
  ...
  void        reset();
  bool        has_registered_cpp_type(pugi::xml_node node) const { return element_types.find(node.name()) != element_types.end(); }
  bool        has_cpp_type(pugi::xml_node node) const { return has_registered_cpp_type(node) || !node.attribute("slot").empty(); }
